...
test('Server Core', async (t) => {
  ...
  await t.test('GET / returns HTML payload', async () => {
    ...
    try {
      ...
      if (serverObj) await serverObj.shutdown();
    }
  });
  ...
  await t.test('GET downloadPath returns encrypted file', async () => {
    ...
    try {
      ...
      const port = serverObj.server.address().port;
      const url = `http://127.0.0.1:${port}${serverObj.downloadPath}`;
      
      const res = await httpClient(url);
      
      assert.strictEqual(res.statusCode, 200);
      assert.strictEqual(res.headers['content-type'], 'application/octet-stream');
      // Original size 1024 + 12 IV + 16 Auth Tag = 1052
      assert.strictEqual(res.headers['content-length'], '1052');
      assert.ok(res.headers['content-disposition'].includes('attachment'));
      assert.strictEqual(res.headers['cache-control'], 'no-store');
      assert.strictEqual(res.body.length, 1052);
